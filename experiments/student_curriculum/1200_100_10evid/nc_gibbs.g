...
Exec( "./saucy test.saucy | sed '$s/,$//' > sym.tmp" );;
...
sample_var := function(var_to_sample)
	...
	end_ind := beg_ind + dom_size[var_to_sample] - 1;;
...
update_marginals := function(n)
	...
	for i in [1..num] do
		for d in [0..(dom_size[i]-1)] do
			marginals[i][d+1] := counts[index]/Float(n);;
			index := index + 1;;
		od;;
	od;;
end;;
		...
		for n in [1..num_sample] do
			...
			if (RemInt(n,inter_sz) = 0) then #or (n<=take_all_till) then
				...
				if iter = iterations then
					...
					AppendTo("nc_kl.csv", String(timeTaken),",",String(klPerInterval[n/inter_sz]),",",String(n),",",String(standard_error),"\n");;
				fi;;
			fi;;
...
PrintTo("nc_marginals.txt", "nc_marginals := [");;

update_marginals(num_sample);;
for i in [1..num] do
	AppendTo("nc_marginals.txt", "\n[");;
	for d in [0..(dom_size[i]-1)] do
		AppendTo("nc_marginals.txt", marginals[i][d+1], ",");;
	od;;
	AppendTo("nc_marginals.txt", "],");;
od;;
